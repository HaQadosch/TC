<script id='GTM-KPS3CD_GATCConf'>
  (function gtm_gatcConf(dl, jQ, cdl, gadl, w, d){
    ...
    if (cdl && gadl && !dl.some(function(a){return /GATC Conf/i.test(a && a.event || '');}) ) try {
      ...
      cdl.error('GTM UK Club1830 GATC Conf: '+e);
    } finally {
      stampEpoch = +new Date();
      dl.push({'event': 'GATC Conf', 'pid': (cdpm.pageid || ''), 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - timeStart});
    }
    return cdl && gadl;
  }(window.dataLayer_KPS3CD, window.jQuery, window.CATTDL, !window.CATTDL?!1:window.CATTDL.DL_gatc, window, document));
</script>