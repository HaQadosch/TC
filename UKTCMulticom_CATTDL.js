...
(function gtm_cattdlCore(jQ, d, w, dl) {
    ...
    var cdpm = {};
    if (jQ && dl) try {
        window.console.clear = function(){ window.console.log('clear'); return true;};
        w.CATTDL = w.CATTDL || {};
        ...
        w.CATTParams && jQ.each(w.CATTParams, function gtm_eachCATTParam(pP){
            ...
            if (!cdpm[pp] || cdpm[pp] !== w.CATTParams[pP]){
                ...
                else if (/lob|holidaytype/i.test(pp)) checked = checked.toLowerCase().replace(/accommodation/, 'hotel').replace(/^package$/, 'epackage') || "generic";
        ...
        dl.push && dl.push({'event': 'core_'+cdpm.pageid});
    }
