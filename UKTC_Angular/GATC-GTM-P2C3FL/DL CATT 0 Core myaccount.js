...
(function gtm_cattdlMyAccount(jQ, d, w, dl){
    ...
    if (jQ && dl && !dl.some(function(a){return /core_my-account/i.test(a && a.event || '')})) try{
        ...
        if (typeof w.CATTParams === 'undefined' && (/\/my-account\/|\/login/).test(location.pathname) || !cdpm.pageid) {cdpm['pageid'] = 'my-account'};
        
        cdpm['user'] = {};
        jQuery.when(jQuery.ajax('/api/users')).done(function(a, b, x){
            cdpm.user.id = ''+(x.responseJSON && x.responseJSON.id || '')
        })

        window.CATTDL.CATTParams = cdpm
    }catch(e){var msg = 'GTM CATTDL Core: '+e; console && ((console.error)?console.error(msg):console.log(msg))}

    dl.push({'event': 'core_my-account'});
    return w.CATTDL     
}(!window.jQuery?!1:window.jQuery, window.document, window, !window.dataLayer?!1:window.dataLayer))
</script>
