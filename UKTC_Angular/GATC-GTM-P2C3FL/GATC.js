...
(function gtm_gatcdlSRP(cdl, dl, loc){
    ...
    return cdl && cdl.CATTParams && window.DL_gatc;
}(window.CATTDL, window.dataLayer || [], document.location || window.location))
//</script>
    ...
    if (cdl && cdl.CATTParams) try {
        ...
        cdl.DL_gatc.webanalytics = {
            ...
            events : {
                ...
                pathfinder            : {'category': 'ThePathfinder', 'action': (/product|ota\/accommodation|srv-accom/i.test(loc.pathname)?'':loc.pathname) || '', 'label': (loc.search?loc.search:"empty"), 'value': 1, 'noninteraction': true}
            }
        };
    } catch(e) {
