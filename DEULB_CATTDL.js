...
(function gtm_cattdlCore(jQ, d, w, dl) {
    ...
    if (jQ && dl) try {
        ...
        w.CATTDL = {
            ...
            post    : function post(key, value) {document.cookie = key+"="+value+"; path=/; domain=.urlaub.de;"},
        ...
        cdpm.lob && w.CATTDL.ckset && w.CATTDL.ckset('gtm_lob', cdpm.lob || '', Infinity, '/', '.urlaub.de');
...
(function gtm_cattdlLP(jQ, dl, cdl) {
    ...
    if (jQ && jQ.extend && cdl) try {
        ...
        if (ck_cdpm.trailingsteps && /\(/i.test(ck_cdpm.trailingsteps)){
         ...
         cdl.ckset('gtm_cdpm', JSON.stringify(keeps), Infinity, '/', '.urlaub.de');
