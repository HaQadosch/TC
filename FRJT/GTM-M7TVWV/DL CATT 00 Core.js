...
(function gtm_cattdlCore(jQ, d, w, dl) {
    ...
    if (jQ && dl) try {
        ...
        w.CATTDL = {
            ...
            post    : function post(key, value) {document.cookie = key+"="+value+"; path=/; domain=.jettours.com;"},
        ...
        w.CATTParams && jQ.each(w.CATTParams, function gtm_eachCATTParam(pP){
            ...
            if (!cdpm[pp] || cdpm[pp] !== w.CATTParams[pP]){
                ...
                else if (/totalprice/i.test(pp)) cdpm['bookingvalue'] = cdpm.bookingvalue || checked;
        ...
        if ((!cdpm.pageid || !/^home$|^subsrch$|^search$|^accom$|^pax$|^pay$|^conf$|^confirmation$|^booking$/i.test(cdpm.pageid)) && cdpm.searches && cdpm.searches.detail) {
            ...
            cdpm.pageid = (/depart-immediat/i.test(pat)?'lastminute':
                                (/derniere-minute/i.test(pat)?'lastminute':
                                    (/zielgebiet/i.test(det)?'subsrch':
                                        (/hotel/i.test(det)?'search':
                                            (/termine/i.test(det)?'accom':
                                                    (/buchung/i.test(det)?'pax':
                                                            (/zahlung/i.test(det)?'pay':
                                                                    (/[^secure\.jettours\.com]/.test(location.hostname) && /[^thomascook_fr2]/.test(pat) && /danke/.test(det)?'secure':'home')
                                                            )
                                                    )
                                            )
                                        )
        ...
        //w.ga('create', 'UA-33870117-1', '.jettours.com', {'name': 'CATTUATC'});
