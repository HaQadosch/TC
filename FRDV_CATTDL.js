...
(function gtm_cattdlCore(jQ, d, w, dl) {
    ...
    if (jQ && dl) try {
        ...
        w.CATTDL = {
            ...
            post    : function post(key, value) {document.cookie = key+"="+value+"; path=/; domain=.voyagesdirects.fr;"},
        ...
        if (!cdpm.pageid || !/^home$|^subsrch$|^search$|^accom$|^pax$|^pay$|^conf$|^confirmation$|^booking$/i.test(cdpm.pageid || '')) {
            var det = cdpm.searches && cdpm.searches.detail;
            cdpm.pageid = /zielgebiet/i.test(det)?'subsrch':/hotel/i.test(det)?'search':/termine/i.test(det)?'accom':/buchung/i.test(det)?'pax':/zahlung/i.test(det)?'pay':/danke/.test(det)?'booking':'home';
        } else if (!cdpm.searches.detail) {
            cdpm.pageid = /depart-immediat/i.test(pat)?'lastminute':(/derniere-minute/i.test(pat)?'lastminute':'home')
        }
