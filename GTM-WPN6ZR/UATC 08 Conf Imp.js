<script id='GTM-WPN6ZR_UATCConfirmationImp'>
  (function gtm_uatcConfirmationImp(jQ, cdl, uadl, w, d, dl){
    'use strict';
    if (cdl && uadl && !dl.some(function(a){return /UATC ConfirmationImp/i.test(a && a.event || '');}) ) try {
      ...
      cdl.error('GTM FR JT MultiTOs UATC ConfirmationImp: '+e);

    } finally {
      var stampEpoch = +new Date();
      dl.push({'event': 'UATC ConfirmationImp', 'pid': (cdpm.pageid || ''), 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - window.dataLayer[0]['gtm.start']})
    }
