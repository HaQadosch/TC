<script id='GTM-WPN6ZR_UATCDLConfirmationSent'>
  (function gtm_uatcdlConfirmationSent(cdl, loc, dl){
    ...
    if (cdl && cdl.CATTParams && !dl.some(function(a){return /UATCDL ConfirmationSent/i.test(a && a.event || '');}) ) try {
      ...
      ld.assign(cdl.DL_uatc, {
        ...
        , webanalytics    : {
            page            : '/vp/fr/'+(cdpm.lob || 'package')+'/'+(cdpm.holidaytype || 'package')+'/'+(cdpm.pageid || 'confirmation')+'_sent'
          , location        : loc && loc.href || window.document && window.document.URL || ''
          , addproduct      : {
              id              : cdpm.accomcode || ''
      ...
      cdl.error('GTM FR JT MultiTO UATCDL ConfirmationSent: '+e)
    } finally {
