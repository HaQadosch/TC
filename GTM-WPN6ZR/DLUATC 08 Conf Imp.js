<script id='GTM-WPN6ZR_UATCDLConfirmationImp'>
  (function gtm_uatcdlConfirmationImp(cdl, loc, dl){
    ...
    if (cdl && cdl.CATTParams && !dl.some(function(a){return /UATCDL ConfirmationImp/i.test(a && a.event || '');}) ) try {
      ...
      ld.assign(cdl.DL_uatc, {
        ...
        , webanalytics    : {
            page            : '/vp/fr/'+(cdpm.lob || 'package')+'/'+(cdpm.holidaytype || 'package')+'/'+(cdpm.pageid || 'confirmation')+'_imp'
          , location        : loc && loc.href || window.document && window.document.URL || ''
          , addproduct      : {
              id              : cdpm.accomcode || ''
      ...
      cdl.error('GTM FR JT MultiTO UATCDL ConfirmationImp: '+e)
    } finally {
