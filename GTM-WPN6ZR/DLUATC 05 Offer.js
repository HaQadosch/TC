<script id='GTM-WPN6ZR_UATCDLOffer'>
  (function gtm_uatcdlOffer(cdl, loc, dl){
    ...
    if (cdl && cdl.CATTParams && !dl.some(function(a){return /UATCDL Offer/i.test(a && a.event || '');}) ) try {
      ...
      ld.assign(cdl.DL_uatc, {
        ...
        , webanalytics    : {
            page            : '/vp/fr/'+(cdpm.lob || 'package')+'/'+(cdpm.holidaytype || 'package')+'/'+(cdpm.pageid || 'offer')
          , location        : loc && loc.href || window.document && window.document.URL || ''
          ...
          , dimensions      : {
            ...
            , dimension106    : {'arrivaltimeoutbound'        : cdpm.arrivaltimeoutbound || 'empty'}
            , dimension107    : {'departuretimeoutbound'      : cdpm.departuretimeoutbound || 'empty'}
            , dimension108    : {'returndate'                 : cdpm.retdate || 'empty'}
            , dimension109    : {'arrivaltimeinbound'         : cdpm.arrivaltimeinbound || 'empty'}
            , dimension110    : {'departuretimeinbound'       : cdpm.departuretimeinbound || 'empty'}
      ...
      cdl.error('GTM FR JT MultiTO UATCDL Offer: '+e)
    } finally {
