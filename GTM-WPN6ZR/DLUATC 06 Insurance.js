<script id='GTM-WPN6ZR_UATCDLInsurance'>
  (function gtm_uatcdlInsurance(cdl, loc, dl){
    'use strict';
    if (cdl && cdl.CATTParams && !dl.some(function(a){return /UATCDL Insurance/i.test(a && a.event || '');}) ) try {
      var ld = cdl._
      ...
      ld.assign(cdl.DL_uatc, {
        ...
        , webanalytics    : {
            page            : '/vp/fr/'+(cdpm.lob || 'package')+'/'+(cdpm.holidaytype || 'package')+'/'+(cdpm.pageid || 'insurance')
          , location        : loc && loc.href || window.document && window.document.URL || ''
      ...
      cdl.error('GTM FR JT MultiTO UATCDL Insurance: '+e)
    } finally {
      var stampEpoch = +new Date();
      dl.push({'event': 'UATCDL Insurance', 'pid': (cdpm.pageid || ''), 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - window.dataLayer[0]['gtm.start']})
    }
    return cdl.DL_uatc
