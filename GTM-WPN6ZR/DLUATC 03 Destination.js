<script id='GTM-WPN6ZR_UATCDLDestination'>
  (function gtm_uatcdlDestination(cdl, loc, dl){
    'use strict';
    if (cdl && cdl.CATTParams && !dl.some(function(a){return /UATCDL Destination/i.test(a && a.event || '');}) ) try {
      var twlh = cdl.twlh
      var twls = cdl.twls
      var ewls = cdl.ewls
      var ld = cdl._
      ...
      ld.assign(cdl.DL_uatc, {
        ...
        , webanalytics    : {
            page            : '/vp/fr/'+(cdpm.lob || 'package')+'/'+(cdpm.holidaytype || 'package')+'/'+(cdpm.pageid || 'destination')
          , location        : loc && loc.href || window.document && window.document.URL || ''
      ...
      cdl.error('GTM FR JT MultiTO UATCDL Destination: '+e)
    } finally {
      var stampEpoch = +new Date();
      dl.push({'event': 'UATCDL Destination', 'pid': (cdpm.pageid || ''), 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - window.dataLayer[0]['gtm.start']})
    }
    return cdl.DL_uatc
