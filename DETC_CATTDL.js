...
(function gtm_cattdlLP(jQ, dl, cdl) {
    ...
    if (jQ && jQ.extend && cdl) try {
        ...
        if (ck_cdpm.trailingsteps && /\(/i.test(ck_cdpm.trailingsteps)){
         ...
         cdl.ckset('gtm_cdpm', JSON.stringify(keeps), Infinity, '/', '.thomascook.de');
...
(function gtm_cattdlAccom(jQ, dl, cdl) {
    ...
    if (jQ && jQ.extend && cdl) try {
        ...
        newPM['starrating'] = cdpm.starrating
         ...
         jQ.extend(cdl.CATTParams, newPM, keeps)
         var domainName = cdl.twlh(/airtours/i)?'.airtours.co.uk':(cdl.twlh(/club18-30/i)?'.club18-30.com':(cdl.twlh(/directholidays/gi)?'.directholidays.co.uk':'.thomascook.de'))
