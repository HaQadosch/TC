...
(function gtm_cattdlCore(jQ, d, w, dl) {
    ...
    if ( jQ && dl && !dl.some(function(a){return /core_/i.test(a && a.event || '');}) ) try {
        ...
        w.CATTDL = {
            ...
            post    : function post(key, value) {document.cookie = key+"="+value+"; path=/; domain=.thomascook.de;";},
        ...
        w.TCDEParams && jQ.each(w.TCDEParams, function gtm_eachTCDEParams(pP){
            var pp = pP.toLowerCase();
            var checked = "";
            if (!cdpm[pp] || cdpm[pp] !== w.TCDEParams[pP]){
                checked = w.TCDEParams[pP];
                if (/string/i.test(typeof checked)) checked = checked.trim();
                if (/lob|holidaytype/i.test(pp)) checked = checked.toLowerCase() || "generic";
                else if (/pageid/i.test(pp)) checked = checked.toLowerCase() || "generic";
                else if (/errorcode/i.test(pp)) checked = checked && checked.substr(0, 270) || "";
        ...
        cdpm.pageid = /package/i.test(cdpm.pageid)?'seo search':(/hotel/i.test(cdpm.pageid)?'seo accom':'') || (/^\/$|^\/pauschalreisen\/?$|^\/hotel\/?$|^\/flug\/?$|^\/staedtereisen\/?$|^\/lastminute\/?$|^\/top-reiseziele\/?$/i.test(location.pathname || '')?'home':'') || (/destinationRegions/i.test(location.pathname || '')?'subsrp':'') || (/accomResult/i.test(location.pathname || '')?'search':'') || (/accomodationDetail/i.test(location.pathname || '')?'accom':'') || (/booking\/index/i.test(location.pathname || '')?'paxpay':'') || (/bookingConfirmation|buchungsbestaetigung/i.test(location.pathname || '')?'booking':'') || cdpm.pageid || 'generic';
...
SRP     http://www.thomascook.de/search/accomResult.jsp       ?page_url_name=pauschalreisen&pageId=package&search=accommodationsearch&alternative=false&sort=recommend&_DARGS=/search/de/destinationRegionsSearchResults.jsp.4_A&_DAV=HOTEL+WAHLEN&_dynSessConf=-533785296000151834&topHotelEnable=&resortId=35&notophotels=&pageId=package&destinationId=35&bannerScriptEnable=&resortName=Mallorca
Accom   http://www.thomascook.de/search/accomodationDetail.jsp?page_url_name=pauschalreisen&pageId=package&search=accommodationdetail&IFF=1835&resortId=35&currentPage=&sort=recommend&sortBy=13
PaxPay  https://www.thomascook.de/booking/index.jsp?resortId=35&currentPage=&page_url_name=pauschalreisen&search=booking&IFF=1835&bookingId=757_0072B0092169A035_0&pageId=package&minDate=Mo.%2018.05.2015&duration=4%A0N%E4chte&sort=recommend
Booking https://www.thomascook.de/booking/fragment/bookingConfirmation.jsp?page_url_name=pauschalreisen&search=bookingconfirm&IFF=1835&bookingId=757_0072B0092169A035_0&pageId=package&resortId=35&currentPage=&frmPaxPage=true&sort=recommend&_requestid=23456
