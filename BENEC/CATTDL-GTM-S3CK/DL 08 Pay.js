...
(function gtm_cattdlPay(jQ, dl, cdl) {
	...
	if (jQ && jQ.extend && cdl) try {
		...
		cdpm['deptairport'] = ctpm.DestinationAirportSelected
		cdpm['retdate'] = +(new Date(ctpm.RetDate && ctpm.RetDate.replace(/(\d+)\/(\d+)\/(\d+)/,'$3-$2-$1')) ||new Date((jQuery('div#foCashTicket').find('div[class="flight-breakdown"]').find('span[class="trip-datetime"]').eq(2).text() || '').trim().replace(/(\d+)\/(\d+)\/(\d+)/,'$3-$2-$1').substring(0,10)) || '0');
		...
		window.gatcDL && gatcDL.push({'event': 'CATTDL Pay'})
	}
