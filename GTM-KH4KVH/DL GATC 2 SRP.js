<script id='GTM-KH4KVH_GATCDLSRP'>
  (function gtm_gatcdlSRP(cdl, dl, loc) {
    'use strict';
    var timeStart = cdl.CATTParams?cdl.CATTParams.timestart:0;
    var cdpm;
    if (cdl && !dl.some(function(a){return /GATCDL SRP/i.test(a && a.event || '');}) ) try {
      var ld = cdl._;
      cdpm = cdl.CATTParams || {};
      ...
      ld.assign(cdl.DL_gatc, {
        ...
        , webanalytics : {
          ...
          , qsp_st : cdl.transpose('ss_st={dest}_{dptd}_{nres}', {
            'dest': destination
            , 'dptd': deptdate
            , 'nres': (cdpm.searchresultstotal || 0) === '0'?'nResults':'yResults'
          }).toLowerCase()
      ...
      cdl.error('GTM UK TC Multicom GATCDL SRP: '+e);
    } finally {
