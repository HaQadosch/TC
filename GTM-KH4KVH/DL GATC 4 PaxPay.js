<script id='GTM-KH4KVH_GATCDLPaxPay'>
  (function gtm_gatcdlPaxPay(cdl, dl, loc) {
    'use strict';
    var timeStart = cdl.CATTParams?cdl.CATTParams.timestart:0;
    var cdpm;
    if (cdl && !dl.some(function(a){return /GATCDL PaxPay/i.test(a && a.event || '');}) ) try {
      var ld = cdl._;
      cdpm = cdl.CATTParams || {};
      ...
      cdl.error('GTM UK TC Multicom GATCDL PaxPay: '+e);
    } finally {
      var stampEpoch = +new Date();
      dl.push({'event': 'GATCDL PaxPay', 'pid': cdl.CATTParams.pageid, 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - timeStart});
    }
    return cdl.DL_gatc;
