...
(function gtm_gatcdlLP(cdl, dl, loc){
	...
	if (cdl && cdl.CATTParams) try {
		...
		cdl.DL_gatc.webanalytics = {
			...
			qsp_p : ( 
				'DeptDate='+(cdl.gadate(cdpm.deptdate) || '').replace(/\//g, '-')+
				'&depAirport='+(cdpm.deptairport || '')+
				'&PaxAdult='+(cdpm.paxadult || '')+
				'&PaxChild='+(cdpm.paxchild || '')+
				'&PaxInfant='+(cdpm.paxinfant || '')+
				'&BoardBasis='+((cdpm.srpfacets && cdpm.srpfacets.boardbasis || []).toString()).replace(/\&amp;/g, '-').replace(/\&/g, '-')+
			...
			events : {
				internalcampaignid	: {'category': 'InternalCampaign', 'action': cdpm.internalcampaignid || '', 'label':loc.href, 'value': 1, 'noninteraction': true},
