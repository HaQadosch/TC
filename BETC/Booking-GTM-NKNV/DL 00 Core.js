    ...
    if (jQ) {
        try{
            ...
            w.CATTDL = {
                ...
                post    : function post(key, value) {document.cookie = key+"="+value+"; path=/; domain=.thomascook.be;"},
            ...
            var domainName = ({
                'thomascook':'.thomascook.be',
                'thomascookairlines':'.thomascookairlines.be'
            })[((/([^\.]*).be|([^\.]*).com|([^\.]*).net/i.exec(location.hostname) || []).filter(function hostname(e){return e}) || ['thomascook']).pop()];
            ...
            cdpm['pageid'] = (({
                ...
                ,'detailsolr'       : 'accom'
                , 'pay'             : 'summary'
            })[ctpm.PageId] || ctpm.PageId || 'booking').toLowerCase();
            ...
            dl.push({'event': 'core_'+(window.CATTDL.CATTParams && window.CATTDL.CATTParams.pageid || 'booking')});
        }
        return w.CATTDL  
    }      
    //}, 1000)
