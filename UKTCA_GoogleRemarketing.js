...
(function gtm_googleRemarketing(dl, cdl, grdl){
    ...
    if (dl && cdl && grdl) try {
        ...
        img.src = '//googleads.g.doubleclick.net/pagead/viewthroughconversion/'+(grdl.conversion_id || '')+'/?value=0&guid=ON&script=0&data='+'holidaytype%3D' +(grdl.holidaytype || '')+'%3Bpageid%3D' +(grdl.pageid || '')+'%3Blob%3D' +(grdl.lob || '')+((/select/i.test(grdl.pageid || ''))?('%3Bduration%3D' +(grdl.duration || '')+'%3Bdeptdate%3D' +(grdl.deptdate || '')+'%3Bdestinationairport%3D' +(grdl.destinationairport || '')+'%3Bdepartureairport%3D' +(grdl.departureairport || '')+'%3Bflighttype%3D' +(grdl.flighttype || '')+'%3Bpaxinfant%3D' +(grdl.paxinfant || '')+'%3Bpaxchild%3D' +(grdl.paxchild || '')+'%3Bpaxadult%3D' +(grdl.paxadult || '')):'')+'&random=' + new Date().getTime();
        grdl.img = {
            src: img.src,
            status : 'fired'
        }
