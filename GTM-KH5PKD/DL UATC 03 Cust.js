<script id='GTM-KH5PKD_UATCDLCust'>
  (function gtm_uatcdlCust(jQ, cdl, loc, dl){
    'use strict';
    var cdpm;
    var timeStart = 0;
    if ( jQ && cdl && cdl.CATTParams && dl) //!dl.some(function(a){return /UATCDL Cust/i.test(a && a.event || '');}) ) 
    try {
      var ld = cdl._;
      cdpm = cdl.CATTParams || {};
      ...
      ld.assign(cdl.DL_uatc, {
        ...
        , webanalytics    : {
          ...
          , addproduct      : ld.assign(window.sessionStorage && JSON.parse(window.sessionStorage.getItem('gtm_srpitems_'+(cdpm.cruiseshipid || cdpm.accomcode || ''))), {
              'price'   : +(cdpm.pricetotal || cdpm.bookingvalue || 0)
            , 'quantity': 1
          })
          , metrics         : {
          ...
          , dimensions      : {
            ...
            // , dimension106    : {'arrivaltimeoutbound'        : arrivaltimeoutbound || ''}
            // , dimension107    : {'departuretimeoutbound'      : departuretimeoutbound || ''}
            , dimension108    : {'returndate'                 : cdpm.retdate || ''}
            // , dimension109    : {'arrivaltimeinbound'         : arrivaltimeinbound || ''}
            // , dimension110    : {'departuretimeinbound'       : departuretimeinbound || ''}
      ...
      cdl.error('GTM UK TC Cruise UATCDL Cust: '+e);
    } finally {
      var stampEpoch = +new Date();
      dl.push({'event': 'UATCDL Cust', 'pid': cdpm.pageid || '', 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - timeStart});
    }
    return cdl.DL_uatc;
