<script id='GTM-KH5PKD_cattdlCore'>
  (function gtm_cattdlLodash(w, d, dl) {
            ...
            if ( jQ && !dl.some(function(a){ return /core_/i.test(a && a.event || ''); }) ) try {
              ...
              if (w.CATTParams) {
                ...
                if (ld && ld.assign) ld.assign(cdpm, ld.transform(w.CATTParams, function gtm_lowerCaseCATTParams(r, n, k){
                  ...
                  else if (/deptdate/i.test(key)) { n = n && +new Date(n.replace(/(\d\d)\/(\d\d)\/(\d\d\d\d)/i, '$3 $2 $1')) ;}
                  else if (/retdate/i.test(key)) { n = n && +new Date(n.replace(/(\d\d)\/(\d\d)\/(\d\d\d\d)/i, '$3 $2 $1')) ;}
              ...
              if (ld && ld.assign) ld.assign(cdpm, {
                ...
                , sessionid         : cdpm.cookies && cdpm.cookies.SESSID || ''
                , timestart         : timeStart
              });
             ...
             cdpm.pageid = ({
                ...
                })[w.CATTParams && w.CATTParams.pageid] || w.CATTParams.pageid || 'home';

              cdpm.poolcontrol = !Boolean((cdpm.utmaguid || 1)%20);
              cdpm.user = cdpm.user || {};
              cdpm.cookiedomain = 'thomascook.com';
              ...
              var msg = 'GTM UK TC Cruise CATTDL Core: '+e;
    ...
    return dl && w.CATTDL;
  }(window, window.document, window.dataLayer_KH5PKD));
</script>