<script id='GTM-KH5PKD_GATCDLCust'>
  (function gtm_gatcdlCust(cdl, dl, loc) {
    'use strict';
    var timeStart = cdl.CATTParams?cdl.CATTParams.timestart:0;
    var cdpm;
    if (cdl &&  dl) //!dl.some(function(a){return /GATCDL Pax/i.test(a && a.event || '');}) ) 
    try {
      var ld = cdl._;
      cdpm = cdl.CATTParams || {};
      ...
      ld.assign(cdl.DL_gatc, {
        ...
        , webanalytics : {
          vp    : cdpmVP
          , qsp_st : cdl.transpose('ss_st={resort}_{name}_{code}', {
            'resort': cdpm.accomresort || ''
            , 'name': cdpm.cruiseship || cdpm.accomname || ''
            , 'code': cdpm.cruisehipid || cdpm.accomcode || ''
          }).toLowerCase()
          ...
          , qsp_p : cdl.transpose('deptdate={dd}&depairport={dp}&paxadult={pa}&paxchild={pc}&boardbasis={bb}&destination={dt}&accomcode={ac}&accomname={an}&rooms={rm}&appserver={as}', {
            ...
            , 'bb'  : boardbasis
            , 'dt'  : destination
            , 'ac'  : cdpm.cruisehipid || cdpm.accomcode || ''
            , 'an'  : cdpm.cruiseship || cdpm.accomname || ''
            , 'rm'  : ''+(cdpm.rooms || '')
            , 'as'  : cdpm.appserver || ''
          }).toLowerCase()
          , customvars : {
      ...
      dl.push({'event': 'GATCDL Cust', 'pid': cdl.CATTParams.pageid, 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - timeStart});
    }
    return cdl.DL_gatc;
