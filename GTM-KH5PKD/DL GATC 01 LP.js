<script id='GTM-KH5PKD_GATCDLLP'>
  (function gtm_gatcdlLP(cdl, dl, loc) {
    'use strict';
    var timeStart = cdl.CATTParams?cdl.CATTParams.timestart:0;
    var cdpm;
    if (cdl && dl && !dl.some(function(a){return /GATCDL LP/i.test(a && a.event || '');}) ) try {
      var ld = cdl._;
      cdpm = cdl.CATTParams || {};
      ...
      cdl.error('GTM UK TC Multicom GATCDL LP: '+e);
    } finally {
      var stampEpoch = +new Date();
      dl.push({'event': 'GATCDL LP', 'pid': cdl.CATTParams.pageid, 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - timeStart});
    }
    return cdl.DL_gatc;
