<script id='GTM-KH5PKD_GATCDLCategorySelection'>
  (function gtm_gatcdlCategorySelection(cdl, dl, loc) {
    'use strict';
    var timeStart = cdl.CATTParams?cdl.CATTParams.timestart:0;
    var cdpm;
    if (cdl && dl) //!dl.some(function(a){return /GATCDL Cust/i.test(a && a.event || '');}) ) 
    try {
      var ld = cdl._;
      cdpm = cdl.CATTParams || {};
      ...
      cdl.error('GTM UK TC Cruise GATCDL CategorySelection: '+e);
    } finally {
      var stampEpoch = +new Date();
      dl.push({'event': 'GATCDL CategorySelection', 'pid': cdl.CATTParams.pageid, 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - timeStart});
    }
    return cdl.DL_gatc;
