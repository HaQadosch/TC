<script id='GTM-KH5PKD_UATCCust'>
  (function gtm_uatcCust(jQ, cdl, uadl, w, dl){
    'use strict';
    var cdpm;
    var timeStart = 0;
    ...
    try {
      ...
      uaProd.brandcode = cdpm.cruiselineid || '';
      cdl.ckset('gtm_uaprod', JSON.stringify(uaProd));

    } catch(e) {
      cdl.error('GTM UK TC Cruise UATC Cust: '+e);
    } finally {
      var stampEpoch = +new Date();
      dl.push({'event': 'UATC Cust', 'pid': cdpm.pageid || '', 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - timeStart});
    }
