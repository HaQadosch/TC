...
var domainName; try {domainName = location.host.match(/tc-ecom.de/gi)?'tc-ecom.de':'thomascook.de'} catch (e){domainName = 'thomascook.de'};
...
window._gaq.push(['CATTGATC._setCustomVar',  23, 'Real URL',  location.protocol + '//' + location.host + location.pathname, 3]); // Tracking URL without get-parameters 02.05.2014 stschaefer
...
(function() {
    ...
    // Thorsten Heissel 20.12.13 new Code for Videotracking
...
var checkedLOB = "generic", checkedHolidayType = "generic", checkedPageID = "accom", checkedDestination = checkedDestinationAirportSelected = checkedAccomName = checkedAccomCode = checkedDeptDate = checkedDepartureAirportSelected = checkedPaxAdult = checkedPaxChild = checkedBoardBasis = checkedRooms = checkedErrorCode = checkedSessID = checkedAppServer = "";
if (typeof window.CATTParams != 'undefined'){
    if (typeof window.CATTParams.LOB != 'undefined') {checkedLOB = window.CATTParams.LOB.toLowerCase()}
    if (typeof window.CATTParams.HolidayType != 'undefined') {checkedHolidayType = window.CATTParams.HolidayType.toLowerCase()}
    if (typeof window.CATTParams.PageID != 'undefined') {checkedPageID = window.CATTParams.PageID.toLowerCase()}
...
(function() {
    ...
    try {
        ...
        if (isFinite(deptDate)) {
            var daysToDeparture = "" + Math.round((deptDate-today) / 1000 / 3600 / 24);
            // console.log("daysToDeparture: " + daysToDeparture);
            // window._gaq.push(['CATTGATC._setCustomVar',  20, 'DaysToDeparture-Accom',  daysToDeparture, 3]);
...
// End new Code Live Chat
            ...
            $("#lightboxGalleryContainer .hotelHeliLink").bind("mousedown",  function() {
                fireHotel(CATTParams.AccomCode, "Helivideo");
            })

            $("#lightboxGalleryContainer .hotelVideoLink").bind("mousedown",  function() {
                fireHotel(CATTParams.AccomCode, "Hotelvideo");
            })
...
// Tracking of Mitarbeiterbuchungen 01.07.2014 stschaefer
try{
    $("#bjForm").submit(function() {
        if($(".errorContainer:visible").length == 0 && $('#voucher0').val().substring(0, 3) == "MAB"){
            window._gaq.push(['CATTGATC._trackEvent', 'Mitarbeiter-Buchung',  "MAB-Buchung", $('#voucher0').val()]);
        }
    });
}catch(e){console && console.log && console.log("DE TC PAXPAY GATC Tag 1 // Mitarbeiterbuchung: "+e)}

try {
    if (window.opCreative){
        window._gaq.push(['CATTGATC._setCustomVar',  41, 'Optimizely',  window.opCreative, 2]);
        window._gaq.push(['CATTGATC._trackEvent', 'Optimizely',  "ERV-Test", window.opCreative, 1, true]);
    }
} catch(e) {}
...
var emptyParams = "";
if (checkedBookingRef == "" || checkedBookingRef == "0") {
    var emptyBookingRef = 0;
    try { emptyBookingRef = (/Auftragsnummer:&nbsp;(.+?)<\/p/gi).exec(document.body.innerHTML)[1]} catch(e) {};
    checkedBookingRef = "empty_"+Math.random();
    emptyParams += ("MissingBookingRef:"+emptyBookingRef+"|");
}
if (checkedBookingValue == "") {
    checkedBookingValue = 0;
    try { checkedBookingValue = (/Gesamtpreis:<\/strong>\&nbsp;\ <strong>(.+?)\ Euro/gi).exec(document.body.innerHTML)[1]} catch(e) {};
    emptyParams += ("MissingBookingValue:"+checkedBookingValue);
}
...
if (document.cookie.indexOf("TC_Ref_" + checkedBookingRef+"="+checkedBookingRef) == -1) {
    ...
    // Setting cookie
    document.cookie = "TC_Ref_" + checkedBookingRef + "=" + checkedBookingRef + "; expires=" + new Date(2099, 11, 31);
}
/*** END Tracking of Booking ********************/


try {
    var tc_ins_price = $("#insuPriceConf").html();
    if (tc_ins_price){
        window._gaq.push(['CATTGATC._trackEvent', 'Optimizely',  "ERV-Test-price", tc_ins_price, 1, true]);
    } else {
        window._gaq.push(['CATTGATC._trackEvent', 'Optimizely',  "ERV-Test-price", "no insurance", 1, true]);
    }
} catch(e) {}
...
if (emptyParams) {
    window._gaq.push(['CATTGATC._trackEvent', 'EmptyParams', emptyParams, 'SessionID='+checkedSessID+'|AppServer='+checkedAppServer, 1, true]);
}
