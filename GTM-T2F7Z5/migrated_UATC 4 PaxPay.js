<script id='GTM-T2F7Z5_UATC2PaxPay'>
  (function gtm_uatcPaxPay(jQ, cdl, uadl, w, dl){
    'use strict';
    if (cdl && uadl && !dl.some(function(a){return /UATC2 PaxPay/i.test(a && a.event || '');}) ) try {
      ...
      cdl.error('GTM UK directholidays UATC2 PaxPay: '+e);
    } finally {
      var stampEpoch = +new Date();
      dl.push({'event': 'UATC2 PaxPay', 'pid': (cdpm.pageid || ''), 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - window.dataLayer[0]['gtm.start']});
    }
