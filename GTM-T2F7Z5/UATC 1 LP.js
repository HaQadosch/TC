<script id='GTM-T2F7Z5_UATCLP'>
  (function gtm_uatcLP(jQ, cdl, uadl, w, d, dl){
    ...
    if (cdl && uadl && !dl.some(function(a){return /UATC LP/i.test(a && a.event || '');}) ) try {
      ...
      cdl.error('GTM UK directholidays UATC LP: '+e);
    } finally {
      var stampEpoch = +new Date();
      dl.push({'event': 'UATC LP', 'pid': (cdpm.pageid || ''), 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - window.dataLayer[0]['gtm.start']});
    }
