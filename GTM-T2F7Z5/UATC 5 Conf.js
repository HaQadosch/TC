<script id='GTM-T2F7Z5_UATCConf'>
  (function gtm_uatcConf(jQ, cdl, uadl, w, dl){
    ...
    if (cdl && uadl && !dl.some(function(a){return /UATC Conf/i.test(a && a.event || '');}) ) try {
      ...
      cdl.error('GTM UK directholidays UATC Book: '+e);
    } finally {
      var stampEpoch = +new Date();
      dl.push({'event': 'UATC Conf', 'pid': (cdpm.pageid || ''), 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - window.dataLayer[0]['gtm.start']});
    }
