<script id='GTM-T2F7Z5_UATCDLPaxPay'>
  (function gtm_uatcdlPaxPay(jQ, cdl, loc, dl){
    ...
    if ( jQ && cdl && cdl.CATTParams && !dl.some(function(a){return /UATCDL PaxPay/i.test(a && a.event || '');}) ) try {
      ...
      cdl.error('GTM UK directholidays UATCDL PaxPay: '+e);
    } finally {
      var stampEpoch = +new Date();
      dl.push({'event': 'UATCDL PaxPay', 'pid': (cdpm.pageid || ''), 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - window.dataLayer[0]['gtm.start']});
    }
    return cdl.DL_uatc;
