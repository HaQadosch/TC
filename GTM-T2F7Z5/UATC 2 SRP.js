<script id='GTM-T2F7Z5_UATCSRP'>
  (function gtm_uatcSRP(jQ, cdl, uadl, w, d, dl){
    ...
    if (cdl && uadl && !dl.some(function(a){return /UATC SRP/i.test(a && a.event || '');}) ) try {
      ...
      cdl.error('GTM UK directholidays UATC SRP: '+e);
    } finally {
      var stampEpoch = +new Date();
      dl.push({'event': 'UATC SRP', 'pid': (cdpm.pageid || ''), 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - window.dataLayer[0]['gtm.start']});
    }
