<script id='GTM-T2F7Z5_UATCDLQuote'>
  (function gtm_uatcdlQuote(jQ, cdl, loc, dl){
    'use strict';
    if ( jQ && cdl && cdl.CATTParams && !dl.some(function(a){return /UATCDL Quote/i.test(a && a.event || '');}) ) try {
      var ld = cdl._;
      ...
      var jqflight = jQ('div.basketSection.packageSection.traditional-package div.CheckoutBasketTable span.basketDetail.flightDetail');
      var departuretimeoutbound = cdl.gadate(new Date(jqflight.eq(0).find('span:eq(1)').text()));
      ...
      ld.assign(cdl.DL_uatc, {
        ...
        , webanalytics    : {
          ...
          , dimensions      : {
            ...
            , dimension106    : {'arrivaltimeoutbound'        : arrivaltimeoutbound || 'empty'}
            , dimension107    : {'departuretimeoutbound'      : departuretimeoutbound || 'empty'}
            , dimension108    : {'returndate'                 : cdpm.retdate || 'empty'}
            , dimension109    : {'arrivaltimeinbound'         : arrivaltimeinbound || 'empty'}
            , dimension110    : {'departuretimeinbound'       : departuretimeinbound || 'empty'}
      ...
      cdl.error('GTM UK directholidays UATCDL Quote: '+e);
    } finally {
      var stampEpoch = +new Date();
      dl.push({'event': 'UATCDL Quote', 'pid': (cdpm.pageid || ''), 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - window.dataLayer[0]['gtm.start']});
    }
    return cdl.DL_uatc;
