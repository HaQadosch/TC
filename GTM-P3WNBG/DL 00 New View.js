<script id='GTM-P3WNBG_cattdlNewView'>
  (function gtm_cattdlNewView(jQ, w, dl, cdl, loc) {
    'use strict';  
      if (jQ && cdl) try {
        ...
        if (!/\/packages\//i.test(locpathname) || (wgD && wgD.details && wgD.matrix)) {
        ...
        if(wgD && wgD.source === 'calendar') {cdpm.pageid = 'accomcalendar'};
        ...
        if(/\/myaccount\/activate/.test(locpathname)) {
          myaccountactivationid = (/\/myaccount\/activate\/(.+)\//.exec(location.pathname) || []).pop() || ''
        };
        cdpm['user'] = cdpm.user || {};
        cdpm.user.id = cdpm.user.id || window.getPageData && window.getPageData('user') && window.getPageData('user').data && window.getPageData('user').data.id || myaccountactivationid || '';
