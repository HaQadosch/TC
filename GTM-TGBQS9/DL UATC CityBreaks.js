    ...
    if ( jQ && cdl && cdl.CATTParams && !dl.some(function(a){return /UATCDL Conf/i.test(a && a.event || '');}) ) try {
      ...
      ld.assign(cdl.DL_uatc, {
        ...
        , webanalytics    : {
          ...
          , addproduct      : {
              id              : cdpm.accomcode || ''
            , name            : cdl.transpose('{accomname}|{destinationairportselected}|{accomcountry}|{accomregion}|{accomresort}', {
      ...
      cdl.error('GTM UK CityBreak UATCDL: '+e);
    } finally {
      var stampEpoch = +new Date();
      dl.push({'event': 'UATCDL Conf CityBreak', 'pid': (cdpm.pageid || ''), 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - window.dataLayer[0]['gtm.start']});
    }
    return cdl.DL_uatc;
