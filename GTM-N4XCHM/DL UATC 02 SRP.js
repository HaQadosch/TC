...
(function gtm_uatcdlSRP(jQ, dl, cdl, loc) {
    ...
    if (jQ && cdl && cdl.CATTParams) try {
        ...
        var vpagepath     = '/vp/'+(cdpm.sitelanguage || '')+'/'+(cdpm.lob || 'lob')+'/'+(cdpm.holidaytype || 'holidaytype')+'/'+(cdpm.pageid || 'pageid');
        ...
        cdl.DL_uatc = {
            ...
            webanalytics    :   {
                ...
                events : {
                    internalcampaignid  : {'category': 'InternalCampaign', 'action': cdpm.internalcampaignid || (cdpm.searches && cdpm.searches.scip || cdpm.searches.s_cip || cdpm.searches.sccl)  || '', 'label': lochref, 'value': 1, 'noninteraction': true, 'page': vppage},
        ...
        var srplist = []; cdsr.forEach(function(e, i){srplist.push({'accomcode': e.accomcode, 'position': cdpm.searchresultspagenbr*1000+i+1})})
        
        /*
        var params = JSON.parse(cdl.ckget('gtm_params') || '{}');
        params.srplist = 'search';
        params.sortoption = cdpm.sortoption || '';
        params.srpresults = srplist || [];
        params.searchresultspagenbr = cdpm.searchresultspagenbr || '';
        CATTDL.ckset('gtm_params', JSON.stringify(params), '', '/', '.thomascook.be');
        */
        ...
        for (var i = 0; i < cdl.DL_uatc.webanalytics.nbrimpressions; i++){
            if (cdsr[i]) {
                ...
                cdl.DL_uatc.webanalytics.addimpression.push({
                    ...
                    category    : ((cdpm.sitelanguage || '')+'|'+(cdpm.lob || "")+'|'+(cdpm.holidaytype || "")).toLowerCase(),
        ...
        cdl.error('GTM BE TC UATCDL SRP: '+e)
    } finally {
