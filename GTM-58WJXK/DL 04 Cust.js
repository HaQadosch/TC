<script id='GTM-58WJXK_cattdlCust'>
(function gtm_cattdlCust(jQ, dl, cdl) {
    ...
    if (jQ && jQ.extend && cdl) try {
        ...
        if (wgdPkg) {
            ...
            for (var i = 0; i < wgdCost.length; i++) {
                curCost = wgdCost[i]
                    ...
                    if (curCost.UIFamily == "INS_TRV" && !/Poliskosten/i.test(curCost.title)) {
                        var ins = {
                            selected    : true,
                            description : curCost.title || '',
                            cost        : curCost.unitCost || 0,
        ...
        window.dataLayer_N5GRGC && dataLayer_N5GRGC.push({'event': 'CATTDL Cust'})
    }
    return cdl
