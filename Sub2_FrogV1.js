...
<script id='gtm_Sub2AT'>
(function gtm_Sub2ATDL(cdl, dl) {
    'use strict'
    if (cdl && dl) try {
        var cdpm = cdl.CATTParams
        var outB = cdpm.flightdetails && cdpm.flightdetails.outbound || ''
        ...
        cdl.DL_sub2 = {
            ...
            script     : {
                status  : 'not fired',
                src     : '//webservices.sub2tech.com/CodeBase/LIVE/Min/sub2.js?LICENSEKEY=2F04A0C9-73F8-482A-BC62-419B39525078&trackPage=Y'
            }
        }
    } catch(e){
        cdl.error("GTM Sub2: "+e)
...
(function gtm_Sub2AT(jQ, cdl, s2dl) {
    'use strict';
    if (jQ && cdl && s2dl && !s2dl.poolControl) try {
        var cdpm = cdl.CATTParams
        ...
        src && jQ.when(jQ.getScript(src)).done(function gtm_sub2Script(){
            ...
            window._gaq && window._gaq.push(function gtm_sub2ControlTest() {
                ...
                var tracker = window._gat._getTrackerByName && window._gat._getTrackerByName('CATTGATC');
                tracker._setCustomVar(45, 'Sub2Live', 'PoolLive', 1);
                tracker._trackEvent('Sub2', 'Live', 'PoolLive', 1, true)
            })
        });
    } catch(e) {
        cdl.error('GTM Sub2: '+e)
    }
    if (s2dl.poolControl) window._gaq && window._gaq.push(function gtm_sub2Control() {
        if (cdl.DL_gatc && cdl.DL_gatc.webanalytics && cdl.DL_gatc.webanalytics.customvars && cdl.DL_gatc.webanalytics.customvars) cdl.DL_gatc.webanalytics.customvars.custvar45 = {'index': 45,'name':'Sub2Live','value':'PoolControl','scope': 1}
        var tracker = window._gat._getTrackerByName && window._gat._getTrackerByName('CATTGATC');
        tracker._setCustomVar(45, 'Sub2Live', 'PoolControl', 1);
        tracker._trackEvent('Sub2', 'Live', 'PoolControl', 1, true)
    })
    return jQ && cdl && s2dl
}(window.jQuery, window.CATTDL, !window.CATTDL?!1:window.CATTDL.DL_sub2))
</script>
