...
(function gtm_cattdlCore(jQ, d, w, dl) {
    ...
    if (jQ && dl) try {
        ...
        w.CATTDL = {
            ...
            twls    : function twls(rg) {return RegExp.prototype.test.call(rg, w.location.href);},
            ewls    : function ewls(rg) {return RegExp.prototype.exec.call(rg, w.location.href);},
            ...
            post    : function post(key, value) {document.cookie = key+"="+value+"; path=/; domain=.thomascook.fr;";},
        ...
        w.tagManageribe && w.tagManageribe.length &&jQ.each(w.tagManageribe[0], function gtm_eachtagManageribe(pP){
            var pp = pP && pP.toLowerCase();
            var checked = "";
            if (!cdpm[pp] || cdpm[pp] !== w.tagManageribe[0][pP]){
                checked = w.tagManageribe[0][pP];
                if (/string/i.test(typeof checked)) {checked = checked.trim()}
                if (/lob|holidaytype/i.test(pp)) checked = checked && checked.toLowerCase() || "generic";
                else if (/LineOfBusiness/i.test(pp)) {checked = checked && checked.replace(/[^_]+_([^_]+)_?.*/, '$1').toLowerCase() || "generic"; pp = 'holidaytype';}
                else if (/typeoffers/i.test(pp)) {checked = checked && checked.toLowerCase() || "generic"; pp = 'lob';}
                else if (/pageid/i.test(pp)) {checked = checked && checked.replace(/REGION/i, 'subsrch').toLowerCase() || "generic";}
                else if (/errorcode/i.test(pp)) {checked = checked && checked.substr(0, 270).replace(/-/, '') || "";}
                else if (/nbrstars/i.test(pp)) {if (checked) cdpm.starrating = checked;}
                else if (/destination|boardbasis|searchresultstop3/i.test(pp)) checked = checked && checked.replace(/\&amp;/g, '-').replace(/\&/g, '-') || "";
                else if (/DeptDate/i.test(pp)) {checked = checked && +new Date(checked.replace(/(\d+)\/(\d+)\/(\d+)/, '$3 $2 $1'))|| '0';}
                else if (/RetDate/i.test(pp)) {checked = checked && +new Date(checked.replace(/(\d+)\/(\d+)\/(\d+)/, '$3 $2 $1'))|| '0';}
                else if (/EndDate/i.test(pp)) {checked = checked && +new Date(checked.replace(/(\d+)\/(\d+)\/(\d+)/, '$3 $2 $1'))|| '0';}
                else if (/price/i.test(pp)) {checked = checked &&  checked.replace(/\D*(\d+\.\d\d).*/, '$1') || '0';}
                else if (/DepartureDate/i.test(pp)) {checked = checked && +new Date(checked.replace(/(\d+)\/(\d+)\/(\d+)/, '$3 $2 $1')) || '0'; pp = 'deptdate';}
        ...
        if ((!cdpm.pageid || !/^home$|^subsrch$|^search$|^accom$|^pax$|^pay$|^conf$|^confirmation$|^booking$/i.test(cdpm.pageid)) && cdpm.searches && cdpm.searches.detail) {
            var det = cdpm.searches.detail;
            cdpm.pageid = /depart-immediat/i.test(pat)?'lastminute':/zielgebiet/i.test(det)?'subsrch':/hotel/i.test(det)?'search':/termine/i.test(det)?'accom':/buchung/i.test(det)?'pax':/zahlung/i.test(det)?'pay':/[^secure\.thomascook\.fr]/.test(location.hostname) && /[^thomascook_fr2]/.test(pat) && /danke/.test(det)?'secure':'home';
        } else if (!cdpm.searches.detail) {
            cdpm.pageid = /depart-immediat/i.test(pat)?'lastminute':'home';
        ...
        if (!jQ) setTimeout(function jQTimeout(){dl.push({'event':'jQTimeOut'});}, 500);
    }
