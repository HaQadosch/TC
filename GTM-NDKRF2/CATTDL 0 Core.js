<script id='GTM-NDKRF2_cattdlCore'>
                ...
                (function gtm_cattdlCore(jQ, d, w, dl, cdl) {
                      if ( jQ && !dl.some(function(a){return /core_/i.test(a && a.event || '')}) ) try {
                          ...
                          if (ld && ld.assign) ld.assign(cdpm, {
                                internalcampaignid: cdpm.searches && cdpm.searches.intcmp || cdpm.hashes && cdpm.hashes.intcmp || ""
                              , emailguid         : cdpm.searches && cdpm.searches.user_id || ""
                              , utmaguid          : cdpm.utmaguid || (/^[^;\.%=]*\.([^;\.%=]*)\./i.exec(cdpm.cookies &&  cdpm.cookies.__utma || ['']) || ['']).pop() || ""
                              , utmbguid          : cdpm.utmbguid || (cdpm.cookies && cdpm.cookies.__utmb) || ""
                              , gaguid            : cdpm.gaguid || (/\.([^\.]*)\.\d*$/i.exec(cdpm.cookies && cdpm.cookies._ga || '') || []).pop() || ""
                              ...
                              , holidaytype       : cdpm.holidaytype || 'flight'
                              ...
                              , landingtimestamp  : (!function(){var d="";return cdl.twdc(/tstp=/i)||(document.cookie="tstp=done",d=new Date(Date.now()).toLocaleString()||"",d=/\//i.test(d)&&(/(\d\d\/\d?\d\/\d\d\d?\d?\s\d\d\:\d?\d\:\d\d)/i.exec(d)&&/(\d\d\/\d?\d\/\d\d\d?\d?\s\d\d\:\d?\d\:\d\d)/i.exec(d).pop()||"")||/(\d?\d\s\w+\s\d\d\d\d\s\d\d:\d?\d:\d\d)/i.exec(d)&&/(\d?\d\s\w+\s\d\d\d\d\s\d\d:\d?\d:\d\d)/i.exec(d).pop()||""),d}())
                              , timestart : timeStart
                          });
