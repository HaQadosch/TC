...
//    window._gaq.push(['_require', 'inpage_linkid', '//www.google-analytics.com/plugins/ga/inpage_linkid.js']);
    window._gaq.push(['CATTGATC._setAccount', 'UA-33999306-1']);

    var domainName = location.host.match(/149\.122\.96\.232/)?'149.122.96.232':(location.host.match(/flythomascook/)?'flythomascook.com':(location.host.match(/thomascookairlines.nl/i)?'thomascookairlines.nl':(location.host.match(/thomascookairlines.fr/)?'thomascookairlines.fr':'thomascookairlines.com')));
    window._gaq.push(['CATTGATC._setDomainName', domainName]);
    window._gaq.push(['CATTGATC._setAllowLinker', true]);
    window._gaq.push(['_gat._anonymizeIp']);
    window._gaq.push(['CATTGATC._addOrganic', 'search.sky.com', 'term'], ['CATTGATC._addOrganic', 'advancedsearch2.virginmedia.com', 'SearchQuery'], ['CATTGATC._addOrganic', 'searchmobileonline.com', 'q'], ['CATTGATC._addOrganic', 'search.orange.co.uk', 'q'], ['CATTGATC._addOrganic', 'search.mywebsearch.com', 'searchfor'], ['CATTGATC._addOrganic', 'search.sweetim.com', 'q'], ['CATTGATC._addOrganic', 'search.alot.com', 'q'], ['CATTGATC._addOrganic', 'search.incredibar.com', 'q'], ['CATTGATC._addOrganic', 'search.bt.com', 'p'], ['CATTGATC._addOrganic', 'searchbrowsing.com', 'q'], ['CATTGATC._addOrganic', 'search.virginmedia.com', 'q']);
    ...
    if (cdl && cdpm) try {
        ...
        if (/holidays/i.test(location.host || document.URL)) {
            ...
            if (/group/i.test(location.pathname || document.URL)) {
                ...
                QSP_ST = 'SS_ST='+
                    ...
                    '&';
                QSP_CAT = 'SS_CAT=holidays_search_uk&';
                QSP_P = '';
                ...
                QSP_ST = 'SS_ST='+
                    ...
                    '|'+(($('input#traveller-hidden-children').val() || '').split(';').length || 0)+
                    '&';
                QSP_CAT = 'SS_CAT=holidays_accom_uk&';
                QSP_P = '';
        ...
        } else if ((/select/i).test(cdpm.pageid) && cdpm.departureairport) {
          	var t = new Date(cdpm.deptdate);
			var formattedDate = t.getFullYear() + '-' + ('0' + (t.getMonth()+1)).slice(-2) + '-' + ('0' + t.getDate()).slice(-2);
            QSP_ST = 'SS_ST='+(cdpm.departureairport || '')+ '-'+(cdpm.returndestinationairport || '')+ '|'+(cdpm.destinationairport || '')+ '-'+(cdpm.returndepartureairport || '')+'|'+(formattedDate || '')+ '|'+(cdpm.flighttype || '')+ '|'+(cdpm.duration || '')+ '|'+(cdpm.paxadult || '0')+ '|'+(cdpm.paxchild || '0')+ '|'+(cdpm.paxinfant || '0')+ '|'+(cdpm.searchresultsoutboundtype ||'')+'-'+(cdpm.searchresultsinboundtype || '')+'&';
