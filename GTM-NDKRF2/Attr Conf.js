  ...
  (function gtm_attribution(dl, cdl){
    ...
    if (dl && cdl) try {
      ...
      if (validLand.length) {
        ...
        dl.push({'event': 'Attribution_'+winningCampaign, 'pid': cdl.CATTParams.pageid, 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - timeStart});
        if (window.sessionStorage && window.sessionStorage.setItem) window.sessionStorage.setItem('gtm_attribution', 'Attribution_'+winningCampaign);
