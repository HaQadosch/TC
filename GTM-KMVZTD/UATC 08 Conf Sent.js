<script id='GTM-KMVZTD_UATCConfirmationSent'>
  (function gtm_uatcConfirmationSent(jQ, cdl, uadl, w, d, dl){
    'use strict';
    if (cdl && uadl && !dl.some(function(a){return /UATC ConfirmationSent/i.test(a && a.event || '');}) ) try {
      ...
      cdl.error('GTM FR TC MultiTOs UATC ConfirmationSent: '+e);

    } finally {
      var stampEpoch = +new Date();
      dl.push({'event': 'UATC ConfirmationSent', 'pid': (cdpm.pageid || ''), 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - window.dataLayer[0]['gtm.start']})
    }
