<script id='GTM-KMVZTD_UATCDestination'>
  (function gtm_uatcDestination(jQ, cdl, uadl, w, d, dl){
    'use strict';
    if (cdl && uadl && !dl.some(function(a){return /UATC Destination/i.test(a && a.event || '');}) ) try {
      ...
      cdl.error('GTM FR TC MultiTO UATC Destination: '+e);
    } finally {
      var stampEpoch = +new Date();
      dl.push({'event': 'UATC Destination', 'pid': (cdpm.pageid || ''), 'timestamp': stampEpoch, 'since gtm.start': stampEpoch - window.dataLayer[0]['gtm.start']})
    }
