<script id='gtm_uatcContacts'>
(function gtm_uatcContacts(jQ, cdl, uadl, w, d, dl){
    'use strict';
    if (cdl && uadl) try {
        ...
        window.ga && window.ga(function gtm_useTracker() {
            ...
            (function gtm_uatcContactAddProduct(){
                ...
                w.ga(trackerName+'ec:setAction','checkout', {'step': 3, 'label': 'contacts'})
                w.ga(trackerName+'send','event', 'ECProductView', uaprod.id,  ''+uaprod.position, 1, eventsendSet, {'nonInteraction': true});
            }());
            ...
            dl.push({'event': 'UATC Contacts'});
            window.gatcDL && gatcDL.push({'event': 'UATC Contacts'});
        })

    } catch(e) {
        cdl.error('GTM BE NEC UATC Contacts: '+e); 
    } finally {     
