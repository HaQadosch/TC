...
(function gtm_uatcLP(jQ, cdl, uadl, w) {
    ...
    if (cdl && uadl) try {
        ...
        window.ga && window.ga(function gtm_useTracker() {
            ...
            $('a.voir.open-contact-form').on('click',function(e){
                var cdpm = cdl.CATTParams || {};
                if (/club-med/.test(location.pathname)) {
                    trc.send('event'
                                ,'quote'
                                , 'demande'
                                , location.pathname
            ...
            $('a.contact-form-button.open-contact-form').on('click',function(e){
                var cdpm = cdl.CATTParams || {};
                if (/club-med/.test(location.pathname)) {
                    trc.send('event'
                                ,'quote'
            ...
            $('button').on('click',function(e){
                var cdpm = cdl.CATTParams || {};
                if (/club-med/.test(location.pathname) && $('div.errors') && ($('div.errors').text()).length > 0) {
                    trc.send('event'
                            ,'quote'
                            , 'envoi'
                            , location.pathname
