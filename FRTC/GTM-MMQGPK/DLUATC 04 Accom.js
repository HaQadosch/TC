...
(function gtm_uatcdlAccom(jQ, dl, cdl, loc) {
    ...
    if (jQ && cdl && cdl.CATTParams) try {
        ...
        cdl.DL_uatc = {
            ...
            webanalytics    : {
                page            : ('/vp/fr/'+((cdpm.lob || 'lob')+'/'+(cdpm.holidaytype || 'holidaytype')+'/'+(cdpm.pageid || 'pageid'))+loc.search).toLowerCase(),
                location        : loc && loc.href || window.document && window.document.URL || '',
                metrics         : {
        ...
        cdl.DL_uatc.webanalytics.addproduct = {
            ...
            variant     : ((cdpm.boardbasis || "")+'|'+(cdpm.deptairport || cdpm.departureairportselected || "")+'|'+(cdl.gadate && cdl.gadate(cdpm.deptdate || 0) || "")+'|'+(cdl.gadate && cdl.gadate(cdpm.retdate || 0) || "")+'|'+(cdpm.paxtotal || "0")+'|'+(cdpm.paxadult || cdpm.srchnbradults || "0")+'|'+(cdpm.paxchild || "0")+'|'+(cdpm.paxinfant || "0")+'|'+(cdpm.pricepp || "0")).toLowerCase(),
